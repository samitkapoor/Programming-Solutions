...
bool getLetter(node *root, string temp)
{
    node *t = root;
...
{
    string temp = ""; // temporary string to store the path to the letter we want to print
    for (int i = 0; i < s.length(); i++)
    {
        temp += s[i];
        bool val = getLetter(root, temp);
        if (val) // if val == true, that means the letter has been found and path needs to be reset
            temp = "";
    }
}
